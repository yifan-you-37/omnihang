...
class Robot:
    def __init__(self,pybullet_api,robot_object_urdf,start_pos=[0.4,0.3,0.4]):
       self.p = pybullet_api
       ...
       self.gripper_left_tip_index = 20#13
       self.gripper_right_tip_index = 17+1
       self.project_dir = "../"
       self.resources_dir = os.path.join(self.project_dir, 'resource')
       self.urdf_dir = os.path.join(self.resources_dir,'urdf')
    #    model_path = os.path.join(self.project_dir, "resource/urdf/franka_panda", robot_object_urdf)
       model_path = robot_object_urdf
       self.configs_dir = os.path.join(self.project_dir,"configs") 
       print("model_path",model_path)

       self.robotId = self.p.loadURDF(model_path, self.start_pos, useFixedBase=True)#, flags=self.p.URDF_USE_SELF_COLLISION and self.p.URDF_USE_SELF_COLLISION_INCLUDE_PARENT) 
       ...
       self.robot_recordings_dir = os.path.join(self.resources_dir,"robot_recordings")
       ...
       self.setJointValue(self.data_q[0],gripper=self.data_gripper[0])
    ...
    def reset(self): 
        ...
        self.setJointValue(qlist[40],glist[40])
        for i in range(40,180,1):
            glist[i] = min(130,glist[i])
            self.jointPositionControl(qlist[i],gripper=glist[i])
    ...
    def operationSpaceTipPositionControl(self,pos,orn=None,null_pose=None,gripperPos=None):
        ...
        if gripperPos is None:
            ...
            gripperForce = [self.gripperMaxForce] * len(self.activeGripperJointIndexList)
            jointPoses = np.array(jointPoses).tolist() 
            self.setJointValue(jointPoses,gripper=gripperPos)
            print(self.p.getLinkState(self.robotId,self.gripper_left_tip_index)[:2])
#            self.p.setJointMotorControlArray(bodyIndex=self.robotId,
#                                        jointIndices=self.controlled_joints + self.activeGripperJointIndexList,
#                                        controlMode=self.p.POSITION_CONTROL,
#                                        targetPositions=jointPoses + self.gripperPos,
