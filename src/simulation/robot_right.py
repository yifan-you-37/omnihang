...
class Robot:
    def __init__(self,pybullet_api,start_pos=[0.4,0.3,0.4]):
       self.p = pybullet_api
       ...
       model_path = os.path.join(self.project_dir, "resource/urdf/franka_panda/panda_robotiq.urdf")
    ...
    def setGripperValue(self, gripper):
        self.gripperOpen = 1 - gripper/255.0
        self.gripperPos = np.array(self.gripperUpperLimitList) * (1 - self.gripperOpen) + np.array(self.gripperLowerLimitList) * self.gripperOpen
        for j in range(6):
            index_ = self.activeGripperJointIndexList[j]
