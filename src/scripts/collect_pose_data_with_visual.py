...
simulation_dir = '../simulation_polish/'
...
class PoseDataCollector:
	...
	def run_collection(self, hook_category, object_category, hook_urdf_dir_arr, object_urdf_dir_arr, exclude_dir, labels_folder_dir, output_dir):
		for i, hook_urdf_dir in enumerate(hook_urdf_dir_arr):
			if not 'hook_wall/145/' in hook_urdf_dir:
				continue
			...
			for j, object_urdf_dir in enumerate(object_urdf_dir_arr):
				if not 'mug/10/' in object_urdf_dir:
					continue
				...
				output_file_pos_name = '{}_{}_{}_{}_pos.txt'.format(hook_category, str(hook_id), object_category, str(object_id))
				output_file_neg_name = '{}_{}_{}_{}_neg.txt'.format(hook_category, str(hook_id), object_category, str(object_id))
				output_file_pos_dir = os.path.join(output_dir, output_file_pos_name)
				output_file_neg_dir = os.path.join(output_dir, output_file_neg_name)
				
				# if os.path.isfile(output_file_dir):
					# continue
				self.collect_data_one_hook_object(hook_bullet_id, object_bullet_id, hook_scaling, object_scaling, hook_world_pos, output_file_pos_dir, output_file_neg_dir)
	...
	def collect_data_one_hook_object(self, hook_bullet_id, object_bullet_id, hook_scaling, object_scaling, hook_world_pos, output_file_pos_dir, output_file_neg_dir):
		...
		for i in range(potential_pos_world.shape[0]):
			...
			for j in range(200):
				...
				object_pos_local = np.array(object_pos_quat_world[0][:3]) - hook_world_pos
				object_quat_local = np.array(object_pos_quat_world[1])
				pose_seq.append(np.append(object_pos_local, object_quat_local))
			...
			if object_pos_quat_world[0][2] > 0.2 and np.linalg.norm(object_pos_quat_world[0][:3]) < 5 \
				...
				np.save('{}_succ_{}.npy'.format(output_file_pos_dir[:-8], success_ct), pose_seq)
				object_quat_local = np.array(object_pos_quat_world[1])
				...
				np.save('{}_fail_{}.npy'.format(output_file_pos_dir[:-8], fail_ct), pose_seq)
				object_quat_local = np.array(object_pos_quat_world[1])
				object_pos_local = global_pos_to_local(object_pos_quat_world[0][:3], hook_world_pos, 1.)
				object_potential_pos_local = global_pos_to_local(potential_pos_world[i], hook_world_pos, 1.)
				with open(output_file_neg_dir, 'a+') as f:
					output_arr = [comma_separated([hook_scaling, object_scaling]), 
									comma_separated(object_potential_pos_local),
									comma_separated(potential_quat[i]),
									comma_separated(object_pos_local),
...
if __name__ == '__main__':
	...
	output_dir = os.path.join('collect_pose_vis')
	...
	hook_urdf_dir_arr, hook_id_arr = get_urdf_dir_from_cat(args.hook_category, hook_dir, 'model_concave_no_wall.urdf', True, exclude_dir, True, labels_folder_dir)
