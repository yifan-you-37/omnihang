...
def train(args, train_set, train_loader, test_set, test_loader, writer, result_folder, file_name, extra_dict=None):
	...
	if not args.run_test:
		...
		import classifier_dataset_new
		train_set_tmp = classifier_dataset_new.MyDataset(args.home_dir_data, train_list_dir, is_train=True, use_partial_pc=True, args=args)
		replay_buffer = ReplayBuffer.combine_replay_buffer(saved_buffer_name_list, train_set_tmp, args.home_dir_data, preload_data=True)
	...
	while True:
		...
		for i, batch_dict in enumerate(loader):
			...
			# training
			for _ in range(5):
				pc_combined_cem_best = cem_search_info_dict['pc_combined_best_all'][0]
				pc_combined, gt_succ_label = replay_buffer.sample(32)
				# print('rotation center', cem_rotation_center_o)
				# print('cem output', cem_out_transl, cem_out_aa)
				# print('final', final_pred_transl, final_pred_aa)
				# # print('tmp', replay_buffer.buffer_fail.pose) 
				# print(np.max(np.abs(pc_o_cem[0] - pc_combined[0][:4096, :3])))
				# print('cem', np.max(np.abs(pc_combined_cem_best[:4096, :3] - pc_combined[0][:4096, :3])))
				# # assert np.allclose(pc_o_cem[0], pc_combined[0][:4096, :3])
				# pc_combined[:16] = pc_combined_cem_best[0:1]
				feed_dict = {
				...
				print('buffer size', replay_buffer.buffer_succ.size, replay_buffer.buffer_fail.size)
				loss_dict = {
				...
				print('training epoch {} iter {}/{} {}'.format(epoch_i, i, epoch_iter, loss_dict_to_str(loss_dict)))
