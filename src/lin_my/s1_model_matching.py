...
def placeholder_inputs(batch_size, num_point, args):
    ...
    pose_ct_pl = tf.placeholder(tf.float32, [None, None], name='pose_ct')
    return pc_o_pl, pc_h_pl, z_pl, gt_obj_transl_pl, gt_obj_aa_pl, pose_mult_pl, pose_ct_pl
...
def get_loss(pred_transl, pred_aa, gt_transl, gt_aa, pose_mult, loss_transl_const=1., end_points=None):
    ...
    return loss_transl, loss_aa, min_idx, loss_all

def get_loss_mult(pred_transl, pred_aa, gt_transl, gt_aa, pose_ct, loss_transl_const=1., end_points=None):
    pred_transl = tf.expand_dims(pred_transl, axis=1)
    pred_aa = tf.expand_dims(pred_aa, axis=1)
    ...
    # return tf.reduce_mean(loss_all * pose_ct)
    loss_tmp = tf.reduce_sum(loss_all * pose_ct, -1)
    pose_ct_tmp = tf.reduce_sum(pose_ct, -1)
    nonzero_mask = tf.math.not_equal(pose_ct_tmp, tf.constant(0, dtype=tf.float32))
    # loss = loss_tmp
    # loss = tf.where(tf.less(tf.reduce_sum(pose_ct, -1), 1e-7), loss_tmp, loss_tmp/tf.reduce_sum(pose_ct, -1))

    loss = tf.boolean_mask(loss_tmp, nonzero_mask) / tf.boolean_mask(pose_ct_tmp, nonzero_mask)
    print('loss multi', loss)
