...
def plot_corr(pc_o, pc_h, pose_transl, pose_quat, cp_top_k_idx_o, cp_top_k_idx_h, corr, aa=False):
	...
	plot_pc(pc_h)
	plot_pc(pc_o_transformed)
	plot_pc(pc_h[cp_top_k_idx_h[0]:(cp_top_k_idx_h[0]+1)], color=(1, 0, 0), scale=0.002)
...
if __name__ == '__main__':
	...
	for i, run_folder_dir in enumerate(glob.glob('{}/*{}'.format(runs_dir, args.exp_name))):
		...
		for result_file_name in eval_result_dict:
			one_result = eval_result_dict[result_file_name][0]
			...
			if 'gt_cp_map_per_o' in one_result:
				...
				gt_gt_cp_corr = create_gt_cp_corr_preload(gt_cp_map_per_o[np.newaxis, :], gt_cp_map_per_h[np.newaxis, :], gt_cp_top_k_idx_o[np.newaxis, :], gt_cp_top_k_idx_h[np.newaxis, :])
			...
			print('loss corr', one_result['loss_corr'], np.sqrt(2 * one_result['loss_corr']))
			if 'gt_cp_map_per_o' in one_result:
			...
			plot_corr(pc_o, pc_h, pred_transl, pred_aa, pred_cp_top_k_idx_o, pred_cp_top_k_idx_h, pred_cp_corr, aa=True)