...
def get_model(point_cloud, l0_points, is_training, bn_decay=None):
    ...
    nor_s = tf_util.conv1d(l0_points, 64, 1, padding='VALID', bn=False, is_training=is_training, scope='fc2',bn_decay=bn_decay)
    nor_s = tf_util.conv1d(nor_s, 4, 1, padding='VALID', bn=False, is_training=is_training, scope='fc2_1', activation_fn=None)
    nor = nor_s[:,:,:3]/(tf.norm(nor_s[:,:,:3],axis=-1,keep_dims=True)+1e-10) 
    end_points['min_dist'] = tf.sigmoid(nor_s[:,:,3]) - 0.4
    end_points['nor'] = nor
    return end_points
